...
# Node selector configuration for pinning brokers/controllers to specific nodes
# Format: "replica-id:node-name,replica-id:node-name,..."
# Example: "broker-0:worker-1,broker-1:worker-2,broker-2:worker-3"
...
validate_storage_configuration() {
  ...
  if [[ "$STORAGE_MODE" == "pvc" ]]; then
    ...
    if [[ -n "$KAFKA_BROKER_NODE_SELECTORS" ]] || [[ -n "$KAFKA_CONTROLLER_NODE_SELECTORS" ]]; then
      info "Storage directories will be needed on the following nodes:"
      local shown_nodes=()
...
prepare_strimzi_hostpath() {
  ...
  # If using node selectors, show which directories are needed on which nodes
  if [[ -n "$KAFKA_BROKER_NODE_SELECTORS" ]] || [[ -n "$KAFKA_CONTROLLER_NODE_SELECTORS" ]]; then
    ...
    if [[ " ${unique_nodes[@]} " =~ " ${current_hostname} " ]]; then
      ...
      if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        sudo mkdir -p "$STRIMZI_HOST_CLUSTER_DIR"
    ...
    # Default behavior - create all directories locally
    info "Creating directories on current node under $STRIMZI_HOST_CLUSTER_DIR"
...
apply_strimzi_resources() {
  ...
  if [[ "$STORAGE_MODE" == "hostPath" ]]; then
    ...
    # Create temporary files for generated PVs
    local controller_pvs_file=$(mktemp)
    local broker_pvs_file=$(mktemp)
    ...
    $KUBECTL_BIN apply -n $NS_KAFKA -f "$controller_pvs_file"

    info "Applying Broker PersistentVolumes..."
    $KUBECTL_BIN apply -n $NS_KAFKA -f "$broker_pvs_file"

    # Clean up temp files
    rm -f "$controller_pvs_file" "$broker_pvs_file"
  else
...
display_connection_info() {
  ...
  # Display node placement configuration if using node selectors
  if [[ -n "$KAFKA_BROKER_NODE_SELECTORS" ]] || [[ -n "$KAFKA_CONTROLLER_NODE_SELECTORS" ]]; then
    info ""
...
main() {
  ...
  if [[ "$STORAGE_MODE" == "hostPath" ]]; then
    ...
    # Show node placement for hostPath mode
    if [[ -n "$KAFKA_BROKER_NODE_SELECTORS" ]] || [[ -n "$KAFKA_CONTROLLER_NODE_SELECTORS" ]]; then
      info ""
      info "  Node Placement:"
      local shown_nodes=()
  ...
  # Validate nodes if using node selectors in hostPath mode
  if [[ "$STORAGE_MODE" == "hostPath" ]] && ([[ -n "$KAFKA_BROKER_NODE_SELECTORS" ]] || [[ -n "$KAFKA_CONTROLLER_NODE_SELECTORS" ]]); then
    validate_nodes
    echo ""
  fi
  ...
  if [[ "$STORAGE_MODE" == "hostPath" ]]; then
    if [[ -n "$KAFKA_BROKER_NODE_SELECTORS" ]] || [[ -n "$KAFKA_CONTROLLER_NODE_SELECTORS" ]]; then
      info "Node selector mode - directories need to be created on specific nodes"
    else
