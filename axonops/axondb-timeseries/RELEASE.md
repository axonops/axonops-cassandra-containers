# AxonDB Time-Series Release Process

This document describes how to create and publish AxonDB Time-Series container releases for both development and production.
...
1. **axondb-timeseries-build-and-test.yml** - Automatic testing on pushes/PRs to `development`
2. **axondb-timeseries-development-publish-signed.yml** - Manual publishing to development registry (Cosign signed)

**Production Workflow:**
1. **axondb-timeseries-build-and-test.yml** - Automatic testing on pushes/PRs to `main`
2. **axondb-timeseries-publish-signed.yml** - Manual publishing to production registry (Cosign signed)
...
The `axondb-timeseries-build-and-test.yml` workflow runs on:
- Push to `development` branch (when `axonops/axondb-timeseries/**` changes)
- Pull requests to `development` (when `axonops/axondb-timeseries/**` changes)
...
- `ghcr.io/axonops/development/axondb-timeseries:5.0.6-1.0.0`
- `ghcr.io/axonops/development/axondb-timeseries:5.0.6`
- `ghcr.io/axonops/development/axondb-timeseries:latest`

**Testing development images:**
```bash
docker pull ghcr.io/axonops/development/axondb-timeseries:5.0.6-1.0.0
docker run -d --name test -p 9042:9042 ghcr.io/axonops/development/axondb-timeseries:5.0.6-1.0.0
...
**Tag naming:** Recommended format: `axondb-timeseries-<version>` (e.g., `axondb-timeseries-1.0.0`)
...
2. Select **AxonDB Time-Series Publish Signed to GHCR** workflow
3. Click **Run workflow** button
4. Fill in inputs:
   - **main_git_tag**: The tag you created (e.g., `axondb-timeseries-1.0.0`)
...
2. **Test** - Runs full test suite on the tagged code
   ...
   - Healthcheck tests
   - System keyspace initialization tests
   - cqlai and cqlsh tests
   - Security scanning (Trivy)

3. **Create Release** - Creates GitHub Release
   - Name: `axondb-timeseries-<container_version>`
   - Tag: `<main_git_tag>`
...
6. **Publish** - Pushes images to GHCR
   - Immutable tag: `5.0.6-1.0.0`
   - Floating tag: `5.0.6` (tracks latest AxonOps for this Cassandra)
...
gh api /orgs/axonops/packages/container/axondb-timeseries/versions | \
...
# Wait for startup
docker logs -f axondb-test

# Test connectivity
docker exec axondb-test cqlai -e "SELECT now() FROM system.local;"
...
gh release view axondb-timeseries-1.0.0
```
...
**Examples:**
- `axondb-timeseries-1.0.0` (recommended)
- `1.0.0`
- `v1.0.0`
...
- `5.0.6-<container_version>` (e.g., `5.0.6-1.0.0`)
...
**Production registry:**
- `ghcr.io/axonops/axondb-timeseries:5.0.6-1.0.0` (immutable)
- `ghcr.io/axonops/axondb-timeseries:5.0.6` (floating)
- `ghcr.io/axonops/axondb-timeseries:latest` (floating)
...
gh release delete axondb-timeseries-1.0.0 --yes

# Delete GHCR package version (via GitHub Packages UI)
# Navigate to: https://github.com/orgs/axonops/packages/container/axondb-timeseries
...
1. Delete the existing GitHub Release: `gh release delete axondb-timeseries-1.0.0`
...
- `1.1.0` - New feature (e.g., new environment variable support)
- `1.1.1` - Bug fix (e.g., healthcheck script fix)
- `2.0.0` - Breaking change (e.g., new Cassandra major version)

### When to Increment

**MAJOR (1.0.0 → 2.0.0):**
- New Cassandra major version (5.0 → 6.0)
...
- Cassandra patch updates (5.0.6 → 5.0.7)
...
- **Major releases:** When Cassandra major versions are released

**Security Updates:**
- Base image (UBI9) updates: Automated in Dockerfile, publish as PATCH
- Cassandra security releases: Publish as PATCH (same major.minor)
- cqlai updates: Publish as MINOR
